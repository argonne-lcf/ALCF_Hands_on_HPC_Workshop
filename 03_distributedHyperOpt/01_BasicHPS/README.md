...
1. Start a `jupyter notebook` (still from `thetalogin` node) via:
...
Another alternative is to copy the notebook and `results.csv` to a local machine. You can use a Jupyter Notebook installation on your laptop to view and analyze the results, after modifying the path variable in the autogenerated notebook.
