...
/* COMPILE:
 *
 * cc -Wall fidgetspinnerA.c -o fidgetspinnerA
 */
...
int main(int argc, char **argv)
{
    ...
    if(argc != 2)
    {
        if(rank == 0)
        {
            fprintf(stderr, "Usage: fidgetspinnerA <directory>\n");
            MPI_Finalize();
            return(-1);
        }
    }
...
static int example1A(const char* dir, int rank, int nprocs)
{
    ...
    sprintf(file_name, "%s/fidgetspinnerA-%d", dir, rank);
	...
	for(j=0; j<nprocs; j++)
	{
	    sprintf(file_name, "%s/fidgetspinnerA-%d", dir, j);
